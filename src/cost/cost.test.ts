...
describe('computeCosts', () => {
  it('computeCosts', () => {
    ...
    expect(computed.evo).toBeCloseTo(13.3);
    expect(computed.modo).toBeCloseTo(1.5 + 2 + 4);
  });
});
...
describe('computeDefaultCost', () => {
  describe('modo', () => {
    ...
    it('1 hour', () => {
      ...
      expect(computeDefaultCost('modo', toHours(1), 10)).toEqual(
    ...
    it('1 hour, 10 minutes (rounds to next 15 min increment)', () => {
      ...
      const distanceCost = 4;
      expect(computeDefaultCost('modo', toHours(1) + 10, 10)).toEqual(
        tripCost + timeCost + distanceCost
      );
    });
    ...
    it('16 hours, 30 minutes', () => {
      ...
      expect(computeDefaultCost('modo', toHours(16) + 30, 100)).toEqual(
        tripCost + timeCost + distanceCost
      );
    });
    ...
    it('2 days, 2 hours, 5 minutes', () => {
      ...
      const distanceCost = 4;
      expect(computeDefaultCost('modo', toDays(2) + toHours(2) + 5, 10)).toEqual(
        tripCost + timeCost + distanceCost
      );
    });
  });
  ...
  describe('evo', () => {
    ...
    it('distance is irrelevant', () => {
      ...
      expect(computeDefaultCost('evo', 60, distance)).toEqual(tripCost + hourlyRate);
    });

    it('1 hour', () => {
      const timeCost = hourlyRate;
      expect(computeDefaultCost('evo', toHours(1), 10)).toEqual(tripCost + timeCost);
    });

    it('1 hour, 10 minutes (does not round)', () => {
      const timeCost = hourlyRate + 10 * minuteRate;
      expect(computeDefaultCost('evo', toHours(1) + 10, 10)).toEqual(tripCost + timeCost);
    });

    it('16 hours, 30 minutes', () => {
      const timeCost = dailyRate; // 16 hours is enough to hit daily rate
      expect(computeDefaultCost('evo', toHours(16) + 30, 10)).toEqual(tripCost + timeCost);
    });

    it('2 days, 2 hours, 5 minutes', () => {
      const timeCost = 2 * dailyRate + 2 * hourlyRate + 5 * minuteRate;
      expect(computeDefaultCost('evo', toDays(2) + toHours(2) + 5, 10)).toEqual(
        tripCost + timeCost
      );
    });
  });
